...
$(document).ready(function () {
    ...
    $("#txt-inquirymsg").keyup(function () {
        validateInquiryMessage();
    });
    $("#txt-inquirymsg").focusout(function () {
        validateInquiryMessage();
    });

    $("#btn-proceed-submit").click(function () {
        if (validateFirstName() == false || validateLastName() == false ||  validateContactNo() == false || validateEmail() == false || validateInquiryMessage() == false ||validateDeliveryAddress01()==false||validateDeliveryAddress04()==false) {
            $("#validation-error-popup").modal('show');
        }else{
            window.location='../../shopping-cart/pages/delivery-summary.html';
        }
    });
});
...
//Validate the contact us-inquiry message
function validateInquiryMessage() {
    if ($("#txt-inquirymsg").val().length == 0) {
        $("#inquirymsg_error").show();
        $("#txt-inquirymsg").css("background-color", background_color);
        $("#inquirymsg_error").text("Inquiry message cannot be null.");
        return false;
    } else {
        $("#txt-inqirymsg").css("background-color", "white");
        $("#inquirymsg_error").hide();
    }
}
